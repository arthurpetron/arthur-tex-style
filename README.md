...
Personal LaTeX document style centered on `arthur-book.cls`. A Tufte-style layout:
margin notes, margin figures, margin citations, margin equation numbers, and
full-width figures.

Extracted from `codynamic-book-machine:docs/arthur_tex_style` so it can be shared
between projects. See `CHANGELOG.md` for what changed on extraction.

## Use as a submodule

```bash
git submodule add https://github.com/arthurpetron/arthur-tex-style.git vendor/arthur-tex-style
```

The class loads its helper modules by relative path (`\RequirePackage{tex/...}`),
so `TEXINPUTS` must include the repository root:

```make
export TEXINPUTS := .:$(abspath vendor/arthur-tex-style):
```
